...
class YouTubeSearchInput(BaseModel):
    ...
    query: str = Field(description="Search term for YouTube videos")
    order: str = Field(default='viewCount', description="Sort order: 'date', 'rating', 'relevance', 'viewCount'")
    duration: str = Field(default='medium', description="Video duration: 'any', 'long' (20+ min), 'medium' (4-20 min), 'short' (<4 min)")
    num_results: int = Field(default=1, description="Number of videos to retrieve. Only specify if user explicitly requests a specific number.")
    before: Optional[str] = Field(default=None, description="Date upper limit in format %m/%d/%Y")
    after: Optional[str] = Field(default=None, description="Date lower limit in format %m/%d/%Y")
...
class GetTranscriptTool(BaseTool):
    """Tool to retrieve YouTube video transcripts and store them in the vector database."""
    name: str = "get_transcript_tool"
    description: str = "Retrieve transcripts for YouTube videos and automatically store them for Q&A."
    yt_instance: ytinteraction = None
    ...
    def _run(self, ids: list[str]) -> str:
        """
        Retrieve transcripts for YouTube video IDs and vectorize them for RAG.
        ...
        try:
            ...
            # Automatically vectorize the transcripts
            if result and any(v.get('transcript') for v in result.values()):
                vectorize_yt_transcripts(result)
                logger.debug("Transcripts fetched and vectorized successfully")
